...
class LegacyParams(ExplicitParams):
    ...
    def transformed_stake(self, stake: Balance) -> Balance:
        lowerbound = self.coefficients.lowerbound
        upperbound = self.coefficients.upperbound
        a = self.coefficients.a
        b = self.coefficients.b

        if lowerbound <= stake <= upperbound:
            return a * stake

        if stake > upperbound:
            return a * upperbound + (stake - upperbound) ** (Decimal(1) / b)
