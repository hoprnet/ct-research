...
async def test_grace_period_under_load(
    ...
    for iteration in range(FLAP_ITERATIONS):
        ...
        flap_subset = list(peers.values())[flapping_peer_count:]
        stress_node.peers = {peer.address.native: peer for peer in flap_subset}
        stress_node.invalidate_peer_cache()  # Must invalidate cache when modifying peers
...
async def test_concurrent_session_creation_race(
    ...
    stress_node.peers = {f"peer_{i}": Peer(f"peer_{i}") for i in range(UNIQUE_RELAYERS)}
